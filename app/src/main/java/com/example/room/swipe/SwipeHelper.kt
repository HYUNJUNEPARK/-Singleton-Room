package com.example.room.swipe
...
class SwipeHelper: ItemTouchHelper.Callback() {  // ItemTouchHelper.Callback 을 구현해야 한다
    ...
    override fun onChildDraw(
        ...
        if(actionState == ItemTouchHelper.ACTION_STATE_SWIPE){
            ...
            val isClamped = getClamped(viewHolder as ListAdapterEx.ViewHolder)
    ...
    override fun getSwipeThreshold(viewHolder: RecyclerView.ViewHolder): Float {   // 터치 유지하다가 떼면 호출됨
        ...
        setClamped(viewHolder as ListAdapterEx.ViewHolder, currentDx <= -clamp)

        return 2f
    }

    private fun getView(viewHolder: RecyclerView.ViewHolder): View {
        return (viewHolder as ListAdapterEx.ViewHolder).itemView.findViewById(R.id.swipe_view) // 아이템뷰에서 스와이프 영역에 해당하는 뷰 가져오기
    }

    private fun setClamped(viewHolder: ListAdapterEx.ViewHolder, isClamped: Boolean){
        viewHolder.setClamped(isClamped)
    }

    private fun getClamped(viewHolder: ListAdapterEx.ViewHolder): Boolean{
    ...
    fun removePreviousClamp(recyclerView: RecyclerView) {
        ...
        previousPosition?.let {
            ...
            setClamped(viewHolder as ListAdapterEx.ViewHolder, false)
